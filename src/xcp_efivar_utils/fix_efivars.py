...
from xcp_efivar_utils.efi import EFI_GLOBAL_VARIABLE, EFI_IMAGE_SECURITY_DATABASE_GUID, EFI_VARIABLE_NON_VOLATILE
from xcp_efivar_utils.utils import unserialize_data, unserialize_struct
from xcp_efivar_utils.xapi import get_pool_certs, get_pool_ref, xapi_session
...
class EfiVariable:
    ...
    def unserialize_variables(buf: bytes, count: int, rem: int):
        for i in range(count):
            ...
            buf, data = unserialize_data(buf, rem, DATA_LIMIT, strict=False)
...
issue.
It also deletes the Secure Boot variable if it contains the varstored-1.2.0-3.1
variables, in order to allow existing VMs to receive dbx updates from Microsoft.
